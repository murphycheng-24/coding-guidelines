...
| 指标 | 原版 4 条规则 | 增强版 10 条规则 | 提升 |
|------|:------------:|:---------------:|:----:|
| **规则数量** | 4 | 10 | **+150%** |
| **工程维度覆盖** | 1/6（16.7%） | 6/6（100%） | **+500%** |
| **LLM 失败模式覆盖** | 3/11（27.3%） | 11/11（100%） | **+267%** |
| **执行阶段** | 1（仅编码） | 4（飞行前→飞行中→飞行后→自审） | **+300%** |

### 按工程维度覆盖情况

| 维度 | 4 条规则 | 10 条规则 |
|------|:-------:|:--------:|
| **编码纪律**（思考、简洁、精准、目标） | ✅ 完整 | ✅ 完整 |
| **验证**（报告前先验证） | ❌ | ✅ 完整 |
| **调试**（系统化排查、先复现再修复） | ❌ | ✅ 完整 |
| **依赖管理**（锁定版本、审计、最小化） | ❌ | ✅ 完整 |
| **沟通**（清晰提交信息、标记隐患） | ❌ | ✅ 完整 |
...
> **结论：** 原版 4 条规则教 LLM *如何写代码*。增强版 10 条规则还教它 *如何验证、调试、沟通和自我审计* — 覆盖的失败模式是原版的 **4 倍**。
